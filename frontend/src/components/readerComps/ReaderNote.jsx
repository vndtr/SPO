...
// В ReaderNote.jsx добавьте в начало:
...
export default function ReaderNote({ 
  ...
  const handleDelete = (e) => {
    ...
    const handleEdit = (e) => {
    e.stopPropagation();
    // Передаем все необходимые данные для редактирования
  ...
  if (type === 'quote') {
    return (
      <div 
        className={`flex flex-col rounded-xl ${getBgColor()} text-blue p-3 mx-2 my-1 relative cursor-pointer hover:shadow-md transition-shadow`}
  ...
  return (
    <div 
      className={`flex flex-col rounded-xl bg-beige-1 text-blue p-3 mx-2 my-1 border-l-4 ${getBorderColor()} relative cursor-pointer hover:shadow-md transition-shadow`}
      onClick={handleCardClick}
    >
      <div className="mb-2">
        <span className="text-xs bg-accent-1/20 text-accent-1 px-2 py-1 rounded-full">
